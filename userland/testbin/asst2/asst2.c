...
main(int argc, char * argv[])
{
        ...
        //Open file
        //Lseek to some position
        //Dup2, check if lseek (newfd, 0, SEEK_CUR) equals to your set position

        int final_pos = lseek(fd, 9, SEEK_SET);
        int new_fd = dup2(fd, 10);
        int new_final_pos = lseek(new_fd, 0, SEEK_CUR);
        if(final_pos != new_final_pos){
          printf("ERROR  new fd is not sharing pointer: %s\n", strerror(errno));
          exit(1);
        } else {
          printf("passed. Error no is: %d", errno);
        }

        char teststr2[] = "This should be printed to stdout!\n";
        int new_stdout = dup2(1, 15);
        int w = write(new_stdout, teststr2, strlen(teststr));
        printf("Wrote %d bytes\n", w);
        printf("* file dup2 okay\n");
        printf("* closing file\n");
        close(fd);
